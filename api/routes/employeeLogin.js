...
router.post('/setup', async (req, res) => {
    ...
    try {
        ...
        const existingAdmin = await EmployeeLogin.findOne({ $or: [{ username }, { email }] });
        if (existingAdmin) {
        ...
        const newAdmin = new EmployeeLogin({
            username,
            email,
...
router.post('/login', [
    ...
    try {
        ...
        const admin = await EmployeeLogin.findOne({ username });
...
router.post('/resetPassword/:resetToken', [
    ...
    try {
        ...
        const admin = await EmployeeLogin.findOne({ resetToken, resetTokenExpires: { $gt: Date.now() } });
