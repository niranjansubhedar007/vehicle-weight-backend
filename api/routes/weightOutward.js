...
const WeightOutward = require('../models/WeightOutward');
...
const updateIsChecked = async (selectedMaterialName) => {
    try {
        ...
        const material = await WeightOutward.findOne({ materialName: selectedMaterialName });
        if (!material) {
...
router.post('/weights', async (req, res) => {
    try {
        ...
        const newWeight = new WeightOutward({
            RSTNo,
            grossWeight,
            tareWeight,
            netWeight,
...
// Add the following route handler for PATCH operation in your Express router:

// router.patch('/weights/:RSTNo', async (req, res) => {
//     try {
//         const { RSTNo } = req.params;
//         const { grossWeight, tareWeight, netWeight } = req.body;

//         // Find the weight entry by its RST No
//         const weight = await Weight.findOne({ RSTNo });
...
router.get('/weights/checked', async (req, res) => {
    try {
        ...
        const weightsWithCheckedMaterials = await WeightOutward.find({ isChecked: true });
...
router.get('/weights/unchecked', async (req, res) => {
    try {
        // Find weights where selectedMaterialName matches the materialName with isChecked true
        const weightsWithCheckedMaterials = await WeightOutward.find({ isChecked: false });
...
router.get('/weight/getNextRSTNo', async (req, res) => {
    try {
        ...
        const maxRSTNo = await WeightOutward.findOne().sort({ RSTNo: -1 }).select('RSTNo');
        let nextRSTNo = 1;
        if (maxRSTNo) {
...
router.get('/weights', async (req, res) => {
    try {
        const weights = await WeightOutward.find();
        res.json(weights);
    } catch (err) {
...
router.delete('/weights/:id', getWeight, async (req, res) => {
    try {
        await res.WeightOutward.remove();
...
async function getWeight(req, res, next) {
    let weight;
    try {
        weight = await WeightOutward.findById(req.params.id);
        if (weight == null) {
            return res.status(404).json({ message: 'Weight not found' });
        }
    } catch (err) {
...
router.get('/RSTNumber/:RSTNumber', async (req, res) => {
    try {
      ...
      const weight = await WeightOutward.findOne({ RSTNumber });
