...
const ProxyWeight = require('../models/ProxyWeight');
...
const updateIsChecked = async (selectedMaterialName) => {
    try {
        ...
        await ProxyWeight.updateMany({ selectedMaterialName }, { isChecked: material.isChecked });
    } catch (error) {
...
router.post('/weights', async (req, res) => {
    try {
        ...
        const newWeight = new ProxyWeight({
            RSTNo,
            grossWeight,
            tareWeight,
            netWeight,
...
router.get('/weights/checked', async (req, res) => {
    try {
        ...
        const weightsWithCheckedMaterials = await ProxyWeight.find({ isChecked: true });
...
router.get('/weights/unchecked', async (req, res) => {
    try {
        // Find weights where selectedMaterialName matches the materialName with isChecked true
        const weightsWithCheckedMaterials = await ProxyWeight.find({ isChecked: false });
...
router.get('/weight/getNextRSTNo', async (req, res) => {
    try {
        ...
        const maxRSTNo = await ProxyWeight.findOne().sort({ RSTNo: -1 }).select('RSTNo');
        let nextRSTNo = 1;
        if (maxRSTNo) {
...
router.get('/weights', async (req, res) => {
    try {
        const weights = await ProxyWeight.find();
        res.json(weights);
    } catch (err) {
...
async function getWeight(req, res, next) {
    let weight;
    try {
        weight = await ProxyWeight.findById(req.params.id);
        if (weight == null) {
...
router.get('/RSTNumber/:RSTNumber', async (req, res) => {
    try {
      ...
      const weight = await ProxyWeight.findOne({ RSTNumber });
