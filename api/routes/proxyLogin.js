...
const ProxyLogin = require('../models/ProxyLogin');
...
router.post('/setup', async (req, res) => {
    ...
    try {
        ...
        const existingAdmin = await ProxyLogin.findOne({ $or: [{ username }, { email }] });
        if (existingAdmin) {
...
router.post('/login', [
    ...
    try {
        ...
        const admin = await ProxyLogin.findOne({ username });
        ...
        if (passwordMatch) {
            const token = jwt.sign
                ({ username: admin.username, role: 'admin' }, SUPERADMIN_JWT_SECRET,);
...
router.post('/resetPassword/:resetToken', [
    ...
    try {
        ...
        const admin = await ProxyLogin.findOne({ resetToken, resetTokenExpires: { $gt: Date.now() } });
