...
const ProxyWeightOutward = require('../models/ProxyWeightOutward');
...
const updateIsChecked = async (selectedMaterialName) => {
    try {
        ...
        const material = await ProxyMaterial.findOne({ materialName: selectedMaterialName });
        if (!material) {
            throw new Error('Material not found');
        }
        // Update the isChecked field in the Weight collection based on the material's isChecked value
        await ProxyWeightOutward.updateMany({ selectedMaterialName }, { isChecked: material.isChecked });
    } catch (error) {
...
router.post('/weights', async (req, res) => {
    try {
        ...
        const newWeight = new ProxyWeightOutward({
            RSTNo,
            grossWeight,
            tareWeight,
            netWeight,
...
router.get('/weights/checked', async (req, res) => {
    try {
        ...
        const weightsWithCheckedMaterials = await ProxyWeightOutward.find({ isChecked: true });
...
router.get('/weights/unchecked', async (req, res) => {
    try {
        // Find weights where selectedMaterialName matches the materialName with isChecked true
        const weightsWithCheckedMaterials = await ProxyWeightOutward.find({ isChecked: false });
...
router.get('/weight/getNextRSTNo', async (req, res) => {
    try {
        ...
        const maxRSTNo = await ProxyWeightOutward.findOne().sort({ RSTNo: -1 }).select('RSTNo');
        let nextRSTNo = 1;
        if (maxRSTNo) {
...
router.get('/weights', async (req, res) => {
    try {
        const weights = await ProxyWeightOutward.find();
        res.json(weights);
    } catch (err) {
...
async function getWeight(req, res, next) {
    let weight;
    try {
        weight = await ProxyWeightOutward.findById(req.params.id);
        if (weight == null) {
...
router.get('/RSTNumber/:RSTNumber', async (req, res) => {
    try {
      ...
      const weight = await ProxyWeightOutward.findOne({ RSTNumber });
